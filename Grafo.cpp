...
void Grafo::agregar_vertice(Lectura* nuevo_vertice) {
    ...
    cout << "\n";
    std::cout << "----------------" << std::endl;
    std::cout << "tamanio: " << vertices->obtener_tamanio() << std::endl;
    for (int i = 1; i <= vertices->obtener_tamanio(); i++){
        vertices->consultar(i)->mostrar_lectura();
    }
    std::cout << "----------------" << std::endl;
    cout << "\n";
    */
...
void Grafo::agregar_camino(Lectura* origen, Lectura* destino, int peso) {
    ...
    // Se podrian sacar las verificaciones estas ya que al crearse el Grafo "automáticamente",
    // basándose en los vértices nunca se va a llegar al caso en donde dichos vértices no existan
