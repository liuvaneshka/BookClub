...
Grafo_lecturas::Grafo_lecturas(Lista_lecturas* lista_lecturas) : Grafo(){
    ...
    this->vertices = new string[lista_lecturas->obtener_tamanio_lecturas()];
...
void Grafo_lecturas::inicializar_vertices(){
    for (int i = 0; i < lista_lecturas->obtener_tamanio_lecturas(); i++)
        vertices[i] = "";
}
...
void Grafo_lecturas::crear_grafo(){
    ...
    while(i <= lista_lecturas->obtener_tamanio_lecturas()){
        ...
        vertices[i - 1] = tipo_vertice;     // Armo un vector con los vertices y sus tipos para solo calcularlos una vez
        i++;
    }
...
void Grafo_lecturas::actualizar_grafo(){
    for(int i = 1; i <= lista_lecturas->obtener_tamanio_lecturas(); i++){
        for (int j = 1; j < i; j++){
            agregar_arista(i, j);
        }
    }
...
void Grafo_lecturas::agregar_arista(int i, int j){
...
string Grafo_lecturas::obtener_tipo(Lectura* lectura_actual){
    string tipo_lectura;

    if(lectura_actual->es_cuento())
        tipo_lectura = CUENTO;
    else if(lectura_actual->es_poema())
        tipo_lectura = POEMA;
    else if(lectura_actual->es_novela())
        tipo_lectura = NOVELA;
    else if(lectura_actual->es_historica())
