    ...
    for (int i = 0; i < cant_vertices; i++){
        for(int j = 0; j < cant_vertices; j++){
            if (matriz_arbol[i][j] != INFINITO){
                ...
                std::cout << origen->obtener_titulo() << " (" << origen->obtener_minutos() << " minutos)"
                          << "  ------" << matriz_arbol[i][j] << "min------>  "
                          << destino->obtener_titulo() << " (" << destino->obtener_minutos() << " minutos)\n" << std::endl;

            }
        }
    }

    std::cout << "El tiempo de lectura es de: " << tiempo_vertices << " minutos" << std::endl;
    std::cout << "El tiempo entre lecturas es de: " << tiempo_aristas << " minutos" << std::endl;
    std::cout << "El tiempo total es de: " << tiempo_vertices + tiempo_aristas << " minutos" << std::endl;
}
